...
class Network(nn.Module):
    def __init__(self,  opt):
        ...
        #---------------------
        # CFNet
        self.CFNet = CrossETDS(num_in_ch=1, num_out_ch=1, upscale=1, num_block=4, num_feat=32, num_residual_feat=3).cuda()
    ...
    def forward(self, zf, y, mask, EA, gt):
        ...
        for i in range(self.depth):
            ...
            # x_play = x.squeeze(1).squeeze(0)
            # x_play = x_play.detach().cpu().numpy() * 255
            # psnr_val0, ssim_val0 = batch_PSNR_ssim(x, gt, 1.)
            # print('stage {}, psnr: {}, ssim {}:'.format(i, psnr_val0, ssim_val0))
            # plt.imshow(x_play)
            # plt.show()
            # cv2.imwrite(r'C:\lz\SAM-NLNet\1\{}_{}_{}.png'.format(i,psnr_val0,ssim_val0), x_play)
