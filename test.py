...
def main(opt):
    ...
    for ii, image_batch in enumerate(loader_val, 0):
        ...
        image_test = image_batch
        image_test = image_test.float()/255
        ...
        zf_input = abs(zf)
        psnr_valzf, ssim_valzf = batch_PSNR_ssim(zf_input, gt, 1.)
        print("PSNR_zf: {}, SSIM_zf: {}".format(psnr_valzf, ssim_valzf))
        # zf_play = zf_input.squeeze(1).squeeze(0)
        # zf_play = zf_play.detach().cpu().numpy() * 255
        # cv2.imwrite(r'C:\lz\SAM-NLNet\1\zf_{}.png'.format(ii), zf_play)
        # plt.imshow(zf)
        # plt.show()
        with torch.no_grad():
            sammap = model_sam(zf_input,ii)
