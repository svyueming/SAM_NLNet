...
class SIMTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=10, shift_size=0,
        ...
        # # 替换CAB
        # self.CrossGL = CrossGL2().cuda()


        self.shortconv = CNet3_new(dim, dim, dim)
        self.re = CNNNet(dim * 2, dim)
    ...
    def forward(self, x, xm, F_h, F_l, mask=None, img_size=(128, 128)):
        B, L, C = x.shape
        H = img_size[0]
        W = img_size[1]
        assert L == W * H, \
        ...
        x = F_h * x + F_l
        # bs,hidden_dim,32x32
        x = torch.cat([x, x_l], dim=1)   # x是transformer的输出，  x_l是cnn的输出
        ...
        # x = self.CrossGL(x, x_l)
        # x = torch.cat([x_cnn, x_trans], dim=1)   # x是transformer的输出，  x_l是cnn的输出
        # x = x_cnn + x_trans  # 1 32 256 256


        # SAM out weights
        # x = F_h * x + F_l  # a=sr   b=se
...
class SIMTransformerBlock_NGZ(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=10, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear', token_mlp='leff',
                 se_layer=False):
...
class BasicShadowFormer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        if cab:
            ...
            self.blocks = nn.ModuleList([
                SIMTransformerBlock_NGZ(dim=dim, input_resolution=input_resolution,
                                    num_heads=num_heads, win_size=win_size,
                                    shift_size=0 if (i % 2 == 0) else win_size // 2,
                                    mlp_ratio=mlp_ratio,
                                    qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward(self, x, xm, F_h, F_l, mask=None, img_size=(128, 128)):
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x = blk(x, xm, F_h, F_l, mask, img_size)
        return x
...
class ShadowFormer(nn.Module):
    def __init__(self, img_size=256, in_chans=1,
        ...
        self.conv1 = BasicShadowFormer(dim=embed_dim,
                                      output_dim=embed_dim,
                                      input_resolution=(img_size,
                                                         img_size),
                                      depth=depths[4],
    ...
    def forward(self, x, xm, F_h, F_l, mask=None):
        # Input  Projection
        xi = torch.cat((x, xm), dim=1)
        self.img_size = (x.shape[2], x.shape[3])
        y = self.input_proj(xi)
        y = self.pos_drop(y)    # LN
        # Bottleneck    #  2 层
        conv0 = self.conv0(y, xm, F_h=F_h, F_l=F_l, mask=mask, img_size=self.img_size)
        conv1 = self.conv1(conv0, xm, F_h=F_h, F_l=F_l, mask=mask, img_size=self.img_size)

        # Output Projection
        y = self.output_proj(conv1, img_size=self.img_size)
...
class SAM(nn.Module):
    ...
    def forward(self, x, ii):  # x 1 1 256 256 (取模值之后） y 1 1 256 256 复数  mask 1 1 256 256
        pic = x * 255
        pic = pic.squeeze(1).squeeze(0).unsqueeze(-1).cpu().detach().numpy().astype(np.uint8)  # 256 256 1
        # plt.imshow(pic.squeeze(-1))
        # plt.show()
        ...
        # plt.imshow(EA*255)
        # plt.show()
        cv2.imwrite(r'C:\lz\SAM-NLNet\1\{}_SAM.png'.format(ii), EA*255*255)
        EA_tensor = torch.from_numpy(EA).unsqueeze(0).unsqueeze(0).cuda()  # 1 1 256 256
        return EA_tensor


class MakeSAM(nn.Module):
    def __init__(self):
        super(MakeSAM, self).__init__()
        self.SAM = SAM().cuda()
    def forward(self, zf, ii=1):
        x = Variable(zf.cuda())  # 1 1 256 256
        EA = self.SAM(x, ii)  # 1 1 256 256
        return EA  # 1 1 256 256
